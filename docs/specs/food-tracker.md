...
```
   ...
   (nutrition source ───────┤           │           │
    ...
    serving_id mapping)     │           │           │
              ...
              │ MCP tools      │  │ :9370       │  │ weekly scheduler  │
...
4. **The UI follows the workout-ui pattern exactly.** `apps/food-ui`,
   Vite + React + Hono, loopback port **9370**, tailscale-serve path mount
...
on `127.0.0.1:9370` (`FOOD_UI_PORT/HOST/BASE_PATH` envs, base-path shim and
...
`config/defaults/startup.yaml` block with `tcp 127.0.0.1:9370` health check,
`tailscale serve --bg --set-path /tango-food http://127.0.0.1:9370` (manual,
...
2. **Food UI** — `apps/food-ui` with Ingredients/Recipes/Planner/Shopping
   list/Prices, hosted at `/tango-food` (9370), reboot-safe.
