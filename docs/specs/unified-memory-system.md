...
| Governance anchor (Slice 2) | **Victor / legal-matter project**, staged after the spine | Legal documents make source-protection + versioning non-negotiable, so they drive the governance design — and transfer directly to [redacted]'s legal use case. Runs on plumbing already proven by the trip. Validated on Devin's private vault; **no legal content enters the repo spec, Linear, or agent context beyond what is operationally necessary**. |
...
- `quick_read` (short current-state summary — [redacted]'s "Quick Read")
...
- **Versioning:** version history for working docs ([redacted]'s legal-draft use case)
