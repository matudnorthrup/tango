...
**Operator profile (Operator mini):** Fleet save behavior — `~/.tango/profiles/default/skills/session-save.md`. Atlas `memory_add` auto-stamps Discord `channel_id` / `thread_id` when the memory MCP receives `TANGO_CONVERSATION_KEY` env (see `packages/discord/src/discord-memory-provenance.ts`).
