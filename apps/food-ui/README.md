...
npm run food-ui:start      # tmux window tango:food-ui, port 9350
npm run food-ui:status
npm run food-ui:restart    # rebuilds first
```

Env: `FOOD_UI_PORT` (9350), `FOOD_UI_HOST` (127.0.0.1), `FOOD_UI_BASE_PATH`
...
```bash
tailscale serve --bg --set-path /tango-food http://127.0.0.1:9350
```
...
```bash
npm run dev:server -w @tango/food-ui   # API on 9350
