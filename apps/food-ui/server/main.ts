...
const PORT = Number(process.env.FOOD_UI_PORT ?? 9350);
...
const TOKEN = process.env.FOOD_UI_TOKEN ?? '';
...
const BASE_PATH = process.env.FOOD_UI_BASE_PATH ?? '/tango-food';

const serverDir = path.dirname(fileURLToPath(import.meta.url));
// dist/server/main.js -> dist/client (built), server/main.ts -> dist/client (dev via tsx)
const clientDir = path.resolve(
  serverDir,
  serverDir.includes(`${path.sep}dist${path.sep}`) ? '../client' : '../dist/client',
);
...
app.get('*', async (c) => {
  ...
  try {
    ...
    try {
      ...
      return c.text('client build not found — run: npm run build -w @tango/food-ui', 404);
    }
  }
});
...
serve({ fetch: fetchWithBasePath, port: PORT, hostname: HOST }, (info) => {
  console.log(`[food-ui] serving on http://${HOST}:${info.port}${BASE_PATH} (client: ${clientDir}, db: ${dbPath})`);
});
