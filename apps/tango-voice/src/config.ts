...
export const config = {
  ...
  sttCommandTailProbeEnabled: parseBooleanWithFallback(process.env['STT_COMMAND_TAIL_PROBE_ENABLED'], true),
  sttCommandTailMs: Math.max(750, parseIntWithFallback(process.env['STT_COMMAND_TAIL_MS'], 2200)),
  sttCommandTailMinDurationMs: Math.max(500, parseIntWithFallback(process.env['STT_COMMAND_TAIL_MIN_DURATION_MS'], 1200)),
