...
describe("project-state integration", () => {
  ...
  it("builds a whisper pointer and a reseed block from the head + open items", () => {
    ...
    storage.upsertProjectState({
      ...
      status: "planning",
      quickRead: "June route locked; [redacted] flight decision open.",
    ...
    expect(block).toContain("[decision] [redacted] flight: Decide whether to move return");
    storage.close();
  });
  ...
  it("reads the live Obsidian body for reseed when a vault root is supplied", () => {
    ...
    fs.writeFileSync(
      ...
      [
        ...
        "## Open Items", "- Book Cortina (pricey in summer)", "- Decide [redacted] return flight", "",
        "## Notes", "misc",
      ].join("\n"),
    );
    ...
    expect(block).toContain("- Decide [redacted] return flight");
