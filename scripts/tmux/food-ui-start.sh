...
if [ ! -f "$APP_DIR/dist/server/main.js" ]; then
  echo "Build output not found at apps/food-ui/dist/server/main.js"
  echo "Run: npm run build -w @tango/food-ui"
  exit 1
fi
...
echo "Started Food UI in tmux target '${SESSION_NAME}:${WINDOW_NAME}' (port ${FOOD_UI_PORT:-9370})"
