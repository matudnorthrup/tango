...
Any time [redacted] asks to create a recipe, modify an existing one, check recipe macros, substitute ingredients, or plan meals using recipes.
...
5. **Add shorthand** — if [redacted] provides a shorthand name, set it on the recipe.
...
When suggesting alternatives, work within [redacted]'s food preferences:
- No added sugar
...
1. **Build a meal plan** — select recipes and meals for the timeframe based on [redacted]'s food preferences, rotation rules, and what's in season/available.
