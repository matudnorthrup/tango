...
- Always call `location_read` first, then route from current position with `osrm_route`.
...
2. Route comparison: generate 2-4 options with `osrm_route`, document distance/time/fuel availability/passes
...
6. Validate: all distances via `osrm_route`, fuel range covers every leg with 50+ mile cushion
